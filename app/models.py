...
class PipelineContext:
    ...
    def load_config(
        ...
        raw = PipelineContext.__read_config_file(filepath_config)

        # Read the file of key words
        raw["key_words"] = load_key_words(filepath_keywords)

        # Extract locations and country for each location
        locations, location_countries = PipelineContext.__extract_locations_countries_from(raw.get("locations", ""))

        # Extract proxies
        proxies = PipelineContext.__extract_proxies_from(raw.get("proxies"))
